        ...
        {choices && sortBy(choices, 'no').map((item, idx) => {
          return (
            <div key={`question-answer-${questionNo}-${item?.no}`} className="flex items-center mb-4">
              <input onClick={(e) => handleClick(item?.no)} id={`default-radio-${questionNo}-${item?.no}`} type="radio" value={`${item?.no}`} name={`${questionNo}`} className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
